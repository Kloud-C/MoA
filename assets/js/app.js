  ...
  if (worldcup) {
    ...
    const showWinner = (winner) => {
      ...
      result.querySelector("[data-share]").addEventListener("click", () => {
        const url = publicPageUrl("worldcup.html");
        window.MOLGGA_SHARE?.open({ title: winner.name, description: winner.detail, text: formatShareText(tr("내가 주말에 하고 싶은 건 [") + tr(winner.name) + tr("] !!"), url), url });
      });
    };
    function reset() {
      round = activities;
