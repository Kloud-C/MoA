(() => {
  ...
  if (worldcup) {
    ...
    const showWinner = (winner) => {
      ...
      back.hidden = false;
      result.hidden = false;
      result.innerHTML = `<article class="archetype-result-card"><div class="archetype-result-card__top"><span class="archetype-result-card__brand">molgga PLAY · 주말 취향 월드컵</span><img class="archetype-result-card__image" src="${winner.image}" alt="${winner.name} 이미지" loading="lazy"><p class="archetype-result-card__label">내가 원하는 주말</p><h3 tabindex="-1">${winner.name}</h3><p class="archetype-result-card__catchphrase">${winner.detail}</p></div><div class="archetype-result-card__body"><p>이번 주말에는 이 시간을 작게라도 일정에 넣어 보세요. 가까운 장소와 부담 없는 시간부터 정하면 바로 시작할 수 있어요.</p></div></article><div class="result-actions"><button class="button button-small" type="button" data-restart>다시 해보기</button><button class="button button-small button-quiet" type="button" data-share>결과 공유 문구 복사</button></div>`;
      ...
      result.querySelector("[data-share]").addEventListener("click", () => copyShareText(formatShareText(tr("내가 주말에 하고 싶은 건 [") + tr(winner.name) + tr("] !!"), "https://molgga.com/worldcup.html"), status));
    };
    function reset() {
      round = activities;
      winners = [];
  ...
  if (animalForm) {
    ...
    animalForm.addEventListener("submit", (event) => {
      ...
      result.querySelector(".archetype-result-card__image").addEventListener("error", (event) => { event.currentTarget.hidden = true; });
      result.querySelector("[data-share]").addEventListener("click", () => copyShareText(formatShareText(tr("이번 테스트 결과는 [") + tr(profiles[winner].name) + tr("] !!"), "https://molgga.com/animal-test.html"), result.querySelector(".share-status")));
      result.hidden = false;
  ...
  if (mbtiForm) {
    ...
    mbtiForm.addEventListener("submit", (event) => {
      ...
      result.querySelector(".archetype-result-card__image").addEventListener("error", (event) => { event.currentTarget.hidden = true; });
      result.querySelector("[data-share]").addEventListener("click", () => copyShareText(formatShareText(tr("나의 MBTI는 [") + summary + tr("] !!"), "https://molgga.com/mbti.html"), result.querySelector(".share-status")));
      result.hidden = false;
